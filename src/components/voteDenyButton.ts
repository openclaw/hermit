...
import { dmUser } from "../lib/notifications.js"
import { writeAuditLog } from "../lib/auditLogger.js"
import { TEAM_DISPLAY_NAMES, type TeamSlug } from "../types/onboarding.js"

export default class VoteDenyButton extends Button {
	customId = "onboarding-vote-deny"
	label = "Deny"
	...
	constructor(applicationId?: string) {
		super()
		if (applicationId) {
			this.customId = `onboarding-vote-deny:applicationId=${applicationId}`
		}
	}
	...
	async run(interaction: ButtonInteraction, data: ComponentData) {
		...
		await db
			...
			.onConflictDoUpdate({
				...
				set: { vote: "DENY", updatedAt: new Date() },
			})
		...
		if (denyCount / totalVotes > threshold) {
			...
			if (teamConfig.trialRoleId) {
				try {
					...
					console.error("[voteDenyButton] Failed to remove trial role:", err)
				}
			}
			...
			await interaction.reply({
				components: [
					new Container(
						[
							new TextDisplay(
								`Your deny vote has been recorded. The deny votes have reached a majority — the application has been declined.`
							),
						],
						{ accentColor: "#ed4245" }
					),
		...
		} else if (approveCount / totalVotes > threshold) {
			// Majority approve still wins even though we just cast a deny
			// Just acknowledge the vote without transitioning
			await writeAuditLog({
				actorId: voterId,
				action: "VOTE_CAST_DENY",
				applicationId,
			...
			await interaction.reply({
				components: [
					new Container(
						[
							new TextDisplay(
								`Your deny vote has been recorded. Current tally: **${approveCount}** approve, **${denyCount}** deny (${totalVotes} total votes cast).`
							),
						],
						{ accentColor: "#f0b132" }
					),
